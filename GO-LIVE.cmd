...
echo [6/17] Preparing allow-listed Cloudflare runtime secrets...
...
echo [10/17] Dry-running the application Worker with production secret names...
...
echo [12/17] Committing and pushing the exact validated source to GitHub main...
...
echo [13/17] Authenticating Supabase CLI...
...
echo [14/17] Linking Supabase and applying database migrations...
call npx supabase link --project-ref "%SUPABASE_PROJECT_REF%" || goto :fail
call npx supabase db push || goto :fail

echo [15/17] Verifying deployed Postgres schema...
call npm run verify || goto :fail

echo [16/17] Deploying GuestAtlas application Worker...
...
echo [17/17] Deploying Cloudflare scheduled maintenance Worker...
